<?php  
		...
		if(!empty(isset($nome)) && filter_input(INPUT_POST, "ipt_nome") !="" && !empty(isset($sobrenome)) && filter_input(INPUT_POST, "ipt_sobrenome") !=""){
			$insert->execute();
			print "<br> Registro Inserido com sucesso...<br>";
			filter_input(INPUT_POST, "ipt_nome")->get
			filter_input(INPUT_POST, "ipt_sobrenome");
...
<body>
	<form method="POST">
		<label>NOME:</label><input type="text" name="ipt_nome" placeholder="Nome" required>
		<br>
		<label>SOBRENOME:</label><input type="text" name="ipt_sobrenome" placeholder="sobrenome" required>
		<br>
